...
<body>	
	...
	<div class="container mt-8">
		<div class="flex-col">
			...
			<h4 class="mb-3" >Updating Product ID: ${product.id}</h4>			
						
			<form:form action="product/edit.htm" method="post" modelAttribute="product" enctype="multipart/form-data" class="flex-col">								 
				 
				 <label for="error" class="form-label error cr"><b>${error}</b></label>
				 
				 <form:input path="id" type="hidden" class="form-control" placeholder="Product ID" />
				...
				<div class="mb-3 w-80">
				  <label for="inventoryNum" class="form-label"><b>Inventory Number</b></label>
				  <span style="border:none;color:red;padding:10px 0 0 5px"><i>${nullMsg}</i></span>
				  <form:input path="inventoryNum" class="form-control" id="inventoryNum" onkeypress="return isNumeric(event)" oninput="checkMaxLength(this)" type="number" min="1" max="5000" placeholder="Enter inventory number"/>				  
				</div>
				...
				<div class="mb-3 w-80">
				  ...
				  <img id="imgshow" src="<c:url value='/images/phone/${product.image}' />" width="200px" height="200px" class="mb-2 ml-2"/>
